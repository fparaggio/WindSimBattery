...
namespace WindSim.Batch.Core.Test
{
    ...
    ///</summary>
    [TestClass()]
    [DeploymentItem("WindSim.Batch.Core.Test\\TestFiles\\270_red_5.xyz")]
    public class Test_XYZFile_270_red_5
    {
        static string fileName = "270_red_5.xyz";
        ...
        [TestMethod]
        public void test_xyz_5_FindResourcefile_Test()
        {
            Assert.IsTrue(System.IO.File.Exists(fileName));
        }

        [TestMethod()]
        public void test_xyz_5_ni_length()
        {  
            Assert.AreEqual(11,file_xyz.ni);
        }

        [TestMethod()]
        public void test_xyz_5_nj_length()
        {
            Assert.AreEqual(5,file_xyz.nj);
        }

        [TestMethod()]
        public void test_xyz_5_nk_length()
        {
        ...
        [TestMethod()]
        public void test_xyz_5_vertical_profile_all()
        {
        ...
        [TestMethod()]
        public void test_xyz_5_y_axis_all()
        {

            double[] orizontal_y_profile = { file_xyz.values[0, 0, 0, 1], file_xyz.values[0, 1, 0, 1], file_xyz.values[0, 2, 0, 1], file_xyz.values[0, 3, 0, 1], file_xyz.values[0, 4, 0, 1] };
            int errors = 0;
            for (int i = 0; i < file_xyz.ni; i++)
            {
                for (int k = 0; k < file_xyz.nk; k++)
                {
                    double[] parsed_orizontal_profile = { file_xyz.values[i, 0, k, 1], file_xyz.values[i, 1, k, 1], file_xyz.values[i, 2, k, 1], file_xyz.values[i, 3, k, 1], file_xyz.values[i, 4, k, 1] };
