...
namespace WindSim.Batch.Core.Test
{
    ...
    ///</summary>
    [TestClass()]
    [DeploymentItem("WindSim.Batch.Core.Test\\TestFiles\\270_red_2.xyz")]
    public class Test_XYZFile_270_red_2
    {
        static string fileName = "270_red_2.xyz";
        ...
        [TestMethod]
        public void test_xyz_2_FindResourcefile_Test()
        {
            Assert.IsTrue(System.IO.File.Exists(fileName));
        }

        [TestMethod()]
        public void test_xyz_2_ni_length()
        {
            Assert.AreEqual(31,file_xyz.ni);
        }

        [TestMethod()]
        public void test_xyz_2_nj_length()
        {
            Assert.AreEqual(6,file_xyz.nj);
        }

        [TestMethod()]
        public void test_xyz_2_nk_length()
        {
            Assert.AreEqual(6,file_xyz.nk);
        }
        ...
        [TestMethod()]
        public void test_xyz_2_vertical_profile_all()
        {
            int x_ran = RandomNumber(0,file_xyz.ni-1);
            int y_ran = RandomNumber(0, file_xyz.nj - 1);
            double[] vertical_profile = { file_xyz.values[x_ran, y_ran, 0, 2], file_xyz.values[x_ran, y_ran, 1, 2], file_xyz.values[x_ran, y_ran, 2, 2], file_xyz.values[x_ran, y_ran, 3, 2], file_xyz.values[x_ran, y_ran, 4, 2], file_xyz.values[x_ran, y_ran, 5, 2]};
            int errors = 0;
            for (int i = 0; i < file_xyz.ni; i++)
            {
                for (int j = 0; j < file_xyz.nj; j++)
                {
                    double[] parsed_profile = { file_xyz.values[i, j, 0, 2], file_xyz.values[i, j, 1, 2], file_xyz.values[i, j, 2, 2], file_xyz.values[i, j, 3, 2], file_xyz.values[i, j, 4, 2], file_xyz.values[i, j, 5, 2]};
        ...
        [TestMethod()]
        public void test_xyz_2_y_axis_all()
        {
